...
class Built_Corpus:
    '''
    Note: The functions in the Built_Corpus class are meant to also
          operate separate from the class, ie, not fitting the class
          variables and only return their specific results. The fit
          method updates all the class variables with
          the returns from each function.

          The Built_Corpus class depends on the functions in pre_clean.py
    '''

    def __init__(self, filepaths=None, total_samples=None):
        '''
        Instantiating the object without fitting gives access to the
        contained methods.

        Initializing with a list of filepaths will trigger the
        .fit() method and populate fields.

        To use a sample value different from the default, it can be
        fed into the constructor.  However, if the object is
        instantiated with a samples value but no filepaths list,
        then the samples field in the object will set to the default
        value in the fit method (all documents) instead of the value
        given to the initializer.

        After object is initialized, call the fit() method to
        populate fields, or use the methods of the object without
        fitting.
        '''
        self.filepaths = None
    ...
    def fit(self, filepaths, total_samples=1200):
        ...
        '''
        print('\nRunning Corpus Compiler from list of data files.')
        start = datetime.now()
